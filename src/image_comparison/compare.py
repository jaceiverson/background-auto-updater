...
def build_images(directory_path: str, file_names: str):
    return [cv2.imread(directory_path + f) for f in file_names]
...
def main(p: str):
    ...
    rprint(f"[yellow]Built images")
    rprint(f"[yellow]Determining pixel uniqueness")
    ...
    t5_files = [f"{p + files[i]}" for i in t5_idx]
    b5_idx = np.argpartition(uniqueness, 5)[:5]
    b5_files = [f"{p + files[i]}" for i in b5_idx]
