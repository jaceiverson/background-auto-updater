...
class SnowbasinImage:
    def __init__(self, background_directory: str, store_previous_images:bool = True) -> None:
        """
    ...
    def get_image(self, url: str, file_path: str) -> bool:
        """
    ...
    def delete_last_image(self,old_file_path: str):
        """
    ...
    def process(self):
        # check the date time
        last_image_time = self.find_last_image_time()
        # unpack the date time
        year, month, day, hour, minute = last_image_time
        # set the file path
        desktop_path = os.path.expanduser(self.background_file_path)
        ...
        image_size = "1080"
        # set the base url
        base_url = f"{self.base_url}/{year}/{month}/{day}/{hour}-{minute}/{image_size}.jpg"
        logger.info(f"Checking for image at [yellow]{base_url}")
        # get the image
        if self.get_image(base_url, os.path.join(desktop_path, file_name)):
            if self.store_previous_images:
                # move the file
                self.move_last_image(old_file_name)
                logger.info(f"Moved {os.path.basename(old_file_name)} to archive folder.")
            else:
                # delete the file
                self.delete_last_image(old_file_name)
                logger.info(f"Deleted {os.path.basename(old_file_name)}")
        else:
